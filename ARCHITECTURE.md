...
| `forge-coordination-archive.md` | Archived coordination entries (older sections) | ✅ Output (if generated) |
...
- **v1.6.2** (2025-03) — Fleet review rounds R18-R19 fixes: Split Relationship validation allows N/A for single-branch headless execution, PowerShell Out-String replaced with [String]::Join to prevent line-wrapping corruption in diffs, Select-Object -ExpandProperty FullName for proper string paths in file discovery, Scribe dispatch includes explicit Split/Iteration fields, forge-coordination-archive.md added to key files table, independent splits rule corrected (sequential in single invocation), plan-verifier ### Warnings block added to output template, deep-review agent separator aligned (colon notation), "medium+" wording updated to "large", slug algorithm example fixed.
- **v1.6.1** (2025-03) — Fleet review rounds R16-R17 fixes: checkpoint tag slug clarified as branch-name derived (not task-name slug), zero-split severity promoted to BLOCKING, convergence checks Execution Config fields, Forge resume multi-state handling, build-fix hard cap documented, fallback model documented, deep-review fix-loop 3-iteration mini-cap (independent of RALPH 10-cap), independent splits clarified as sequential in single invocation, single-branch git fetch added, code agent CREATE/MODIFY tool guidance, verifier iteration numbers in output headers, plan-verifier WARNING verdict clarified (does not affect binary APPROVED/ISSUES FOUND), Split Relationship conditional on multi-split, Split Relationship validation allows N/A for single-branch, Scribe dispatch includes explicit Split/Iteration fields, forge-coordination-archive.md listed in key files table.
